...
begin
  ...
  begin
    ...
    else if ListBox1.Items[I] = 'Л' then LeftButtonClick(nil)
    else if ListBox1.Items[I] = 'П' then RightButtonClick(nil)
    else if ListBox1.Items[I] = 'C' then Button1Click(nil);

    if hasError then
    begin
      ShowMessage('Произошла ошибка при выполнении действия под номером ' + IntToStr(I+1) + ', программа остановлена');
      Break;
    end;
  ...
  if hasError = false then
    ShowMessage('Программа выполнена, вы можете сохранить программу, либо написать новую нажав кнопку "Сброс"');
end;
...
begin
  ...
  if needLogCommands then
  begin
    ListBox1.Items.Add('Л');
    if Sender <> nil then
      ListBox2.Items.AddPair(IntToStr(ListBox2.Count+1), 'Л');
  end;
...
begin
  ...
  begin
    ...
    if ListBox1.Items[I] = 'Л' then LeftButtonClick(nil);
    if ListBox1.Items[I] = 'П' then RightButtonClick(nil);
    if ListBox1.Items[I] = 'C' then Button1Click(nil);
  end;

  ListBox2.Items.AddPair(IntToStr(ListBox2.Count+1), 'Ц' + ' ' + minLoop.Text + ' ' + maxLoop.Text);
end;
...
begin
  ...
  begin
    if QCar > 0 then
      ...
      begin
        ShowMessage('Дествие невозможно выполнить');
        hasError := true;
        exit;
      end;
...
begin
  if needLogCommands then
  begin
    ListBox1.Items.Add('П');
    if Sender <> nil then
      ListBox2.Items.AddPair(IntToStr(ListBox2.Count+1), 'П');
  end;
